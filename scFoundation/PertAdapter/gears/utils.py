...
'''
def loss_adapt(pred, y, mean_ctrl, std_ctrl, thresh_lmbd = 2):
    #pred: B, N
    #y: B, N
    #mean_ctrl: N,
    #std_ctrl: N,
    std_ctrl_mask  = (std_ctrl > 1e-2)
    mean_ctrl = mean_ctrl[std_ctrl_mask].reshape((1, -1))
    std_ctrl = std_ctrl[std_ctrl_mask].reshape((1, -1))
    
    pred_dev = (pred[:, std_ctrl_mask] - mean_ctrl)/std_ctrl
    y_dev = (y[:, std_ctrl_mask] - mean_ctrl)/std_ctrl

    loss_mask  = torch.logical_or((pred_dev ** 2) > thresh_lmbd ** 2, (y_dev ** 2) > thresh_lmbd ** 2)

    return torch.sum(((pred_dev - y_dev)**2) * loss_mask)/pred_dev.shape[0]/pred_dev.shape[1]
    pass'''

def loss_adapt_old(pred, y, perts, adata, geneid2idx, pert2pert_full_id, dict_filter = None, de_loss_weight = 0.5):
    #pred: B, N
    #y: B, N
    mse_p = torch.nn.MSELoss()
    perts = np.array(perts)
    losses = torch.tensor(0.0, requires_grad=True).to(pred.device)
    assert (de_loss_weight > 0 and de_loss_weight < 1), "de_loss_weight should be between 0, 1!"

    for pert in set(perts):
        pert_idx = np.where(perts == pert)[0]
        ...
        if pert != 'ctrl':
            ...
            pred_p_de = pred[pert_idx][:, de_idx]
            y_p_de = y[pert_idx][:, de_idx]

            retain_idx = dict_filter[pert]
            pred_p = pred[pert_idx][:, retain_idx]
            y_p = y[pert_idx][:, retain_idx]
            losses += de_loss_weight * mse_p(pred_p_de, y_p_de) + (1- de_loss_weight) * mse_p(pred_p, y_p)
        else:
...
def loss_adapt(pred, y, perts, adata, geneid2idx, pert2pert_full_id, dict_filter = None, de_loss_weight = 0.5):
    #pred: B, N
    #y: B, N
    mse_p = torch.nn.MSELoss()
    perts = np.array(perts)
    losses_de = torch.tensor(0.0, requires_grad=True).to(pred.device)
    losses_mse = torch.tensor(0.0, requires_grad=True).to(pred.device)
    assert (de_loss_weight > 0 and de_loss_weight < 1), "de_loss_weight should be between 0, 1!"
    ...
    for pert in set(perts):
        ...
        if pert != 'ctrl':
            ...
            pred_p = pred[pert_idx][:, retain_idx]
            y_p = y[pert_idx][:, retain_idx]
            losses_de += mse_p(pred_p_de, y_p_de) 
            losses_mse +=  mse_p(pred_p, y_p)
        else:
            pred_p = pred[pert_idx]
            y_p = y[pert_idx]
            losses_mse += mse_p(pred_p, y_p)
    losses_de = losses_de/(len(set(perts)))
    losses_mse = losses_mse/(len(set(perts)))

    losses_de_value = losses_de.detach()
    losses_mse_value = losses_mse.detach()
    if losses_de_value > 0:
        de_loss_weight = (losses_de_value + losses_mse_value) / losses_de_value
    else:
        de_loss_weight = 0
    ...
    losses = de_loss_weight * losses_de + mse_loss_weight * losses_mse
    return losses
