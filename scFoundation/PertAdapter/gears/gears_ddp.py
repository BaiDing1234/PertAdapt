...
from .model_new import GEARS_Model_Pert_Adapter_New
...
class GEARSAdapt_ddp:
    def __init__(self, pert_data, 
        ...
        if self.is_master and self.weight_bias_track:
            import wandb
            if api_key != "None":
        ...
        ######
        #self.pert_idx_to_gene_idx = {}
        #for pert_name, pert_idx in self.node_map_pert.item():
        #    self.pert_idx_to_gene_idx[pert_idx] = pert_data.node_map.get(pert_name, -1)
        ######
    ...
    def model_initialize(self, hidden_size = 64,
        ...
        model_class = self.config.get('model_class', 'GEARS_Model_Pert_Adapter_New')
        
        if model_class == 'GEARS_Model_Pert_Adapter_New':
            self.model = GEARS_Model_Pert_Adapter_New(self.config).to(self.device)
    ...
    def train(self, epochs = 20, result_dir='./results',
        ...
        if self.config['finetune_method'] == 'frozen':
            for pname, param in self.model.named_parameters():
                '''
                if "singlecell_model" in pname:
                    if 'adapter' in pname or 'to_final' in pname:
                ...
                if param.requires_grad:
                    if self.is_master:
                        print(f"parameter: {pname}, #params: {int(param.numel())}")
        ...
        for epoch in range(epochs):
            ...
            self.model.train()
            if self.config['finetune_method'] == 'frozen':
                if hasattr(self.model.module, 'singlecell_model'):
                    self.model.module.singlecell_model.eval()
            dist.barrier()
            ...
            for step, batch in enumerate(train_loader):
                ...
                batch.to(self.device)
                y = batch.y
                if (step+1) % GRADIENT_ACCUMULATION != 0 and (step+1!=len(train_loader)):
                    with self.model.no_sync():
                        pred = self.model(batch)
                        if self.loss == 'loss_fct':
                        ...
                        elif self.loss == 'loss_adapt':
                            loss = loss_adapt(pred, y, batch.pert, self.adata, self.geneid2idx, 
                                              self.pert2pert_full_id, self.dict_filter, 
                                              de_loss_weight=self.config['de_loss_weight'])
                        loss = ddp_loss_weight * loss / GRADIENT_ACCUMULATION 
                        loss.backward()
            ...
            dist.barrier()
            scheduler.step()
            if ((epoch + 1) % valid_every == 0) and (epochs + 1 >= begin_valid_epoch):
            ...
            if valid_every > epochs and self.is_master:
                self.save_model(result_dir)
        dist.barrier()
